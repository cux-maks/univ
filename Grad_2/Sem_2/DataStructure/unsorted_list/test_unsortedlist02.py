...
# @version 2022년도 2학기
# @author 김상진
# @file test_unsortedlist01.py
# 용량 고정, 중복 허용, 배열을 이용한 비정렬 정수 리스트
# 테스트 프로그램

import unittest
from unsortedlist02 import UnsortedList

class TestUnsortedList(unittest.TestCase):

	#@unittest.skip
	def test_listinit_2(self):
		L = UnsortedList()
	...
	def test_add(self):
		...
		self.assertListEqual(output, [5, 3, 2, 1])
	...
	def test_peek(self):
		...
		while L:
			output.append(L.popfront())
		self.assertListEqual(output,[7,5,3,1,2])
	...
	def test_setitem(self):	
		...
		L[1:3] = [2,2,2]
		output = [x for x in L]
		self.assertListEqual(output, [1,2,2,2,3,0,4,0,5,0])
		self.assertEqual(len(L), 10)
